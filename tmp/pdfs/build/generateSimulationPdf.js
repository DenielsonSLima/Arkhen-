...
const safeAddImage = (doc, dataUrl, x, y, width, height) => {
    if (!dataUrl)
        return false;
    try {
...
const drawWatermark = (doc, input) => {
    const watermark = input.watermark;
    if (!watermark?.enabled || !watermark.dataUrl)
        return;
...
const drawFooter = (doc, input, pageNumber, totalPages) => {
...
const drawPageHeader = (doc, input, firstPage) => {
    ...
    doc.setFontSize(13);
    doc.setTextColor(NAVY);
    const companyLines = doc.splitTextToSize(companyName.toUpperCase(), 105);
...
const measureRow = (doc, row) => {
    doc.setFontSize(8.5);
    const labelLines = doc.splitTextToSize(row.label, 76);
    const valueLines = doc.splitTextToSize(row.value || '—', 91);
    return {
        labelLines,
        valueLines,
...
export const generateSimulationPdf = async (input) => {
    ...
    input.sections.forEach((section) => {
        ...
        if (y + 12 + firstRowHeight > CONTENT_BOTTOM)
            addPage();
        doc.setFillColor('#f8fafc');
        ...
        section.rows.forEach((row) => {
            ...
            if (y + measured.height > CONTENT_BOTTOM)
                addPage();
            doc.setFontSize(8.5);
            doc.setFont('helvetica', 'normal');
            doc.setTextColor(SLATE);
    ...
    doc.setFontSize(7.7);
    doc.setTextColor(SLATE);
    doc.text(doc.splitTextToSize('Resultado estimado com os parâmetros informados. Confira documentos, regras e obrigações oficiais antes de qualquer apuração ou transmissão.', CONTENT_WIDTH - 8), MARGIN_X + 4, y + 6, { lineHeightFactor: 1.25 });
...
export const pdfBytesToDataUrl = (bytes) => {
    let binary = '';
