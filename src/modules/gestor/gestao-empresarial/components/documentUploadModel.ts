...
export const ALLOWED_ACCOUNTING_EXTENSIONS = [
  ...
  '.xml', '.txt', '.efd', '.ecd', '.ecf', '.ofx', '.qif', '.rem',
  '.ret', '.cnab', '.png', '.jpg', '.jpeg', '.webp', '.zip', '.rar',
  '.7z', '.pfx', '.p12', '.cer', '.crt', '.pem', '.p7s', '.key',
  '.eml', '.msg',
];
...
const readDirectoryEntries = (entry: DragFileSystemEntry) => (
  new Promise<DragFileSystemEntry[]>((resolve, reject) => {
...
const collectEntryFiles = async (
  ...
  const nested = await Promise.all(
    children.map((child) => collectEntryFiles(child, `${parentPath}${entry.name}/`)),
  );
  return nested.flat();
};
