...
interface AddCnaeFormProps {
  onSave: (cnae: Cnae) => void;
  onCancel: () => void;
  isSaving: boolean;
}

export const AddCnaeForm: React.FC<AddCnaeFormProps> = ({ onSave, onCancel, isSaving }) => {
  const [codigo, setCodigo] = useState('');
  const [descricao, setDescricao] = useState('');
  const [simplesNacional, setSimplesNacional] = useState(true);
  const [simplesAnexo, setSimplesAnexo] = useState<Cnae['simplesAnexo']>('Anexo III');
  const [presuncaoIrpj, setPresuncaoIrpj] = useState('32.00');
  const [presuncaoCsll, setPresuncaoCsll] = useState('32.00');
    ...
    <form onSubmit={handleSubmit} className="config-form popup-form">
      ...
      <div className="popup-form-buttons">
        ...
        <button type="submit" className="btn-invite" disabled={isSaving}>
          {isSaving ? 'Salvando...' : 'Adicionar CNAE'}
        </button>
      </div>
    </form>
  );
