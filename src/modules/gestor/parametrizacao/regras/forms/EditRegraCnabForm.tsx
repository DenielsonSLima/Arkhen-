...
interface EditRegraCnabFormProps {
  regra: RegraCnab;
...
export const EditRegraCnabForm: React.FC<EditRegraCnabFormProps> = ({
  regra,
  onSave,
  onCancel,
  isSaving,
  ...
  const [banco, setBanco] = useState(regra.banco);
  const [tipoRegra, setTipoRegra] = useState<'cobranca' | 'conciliacao'>(regra.tipoRegra);

  // Cobrança parameters
  const [multa, setMulta] = useState(regra.multa?.toFixed(2) || '2.00');
  const [juros, setJuros] = useState(regra.juros?.toFixed(2) || '1.00');
  const [diasTolerancia, setDiasTolerancia] = useState(regra.diasTolerancia?.toString() || '0');

  // Conciliação parameters
  const [padraoTexto, setPadraoTexto] = useState(regra.padraoTexto || '');
  const [contaContabil, setContaContabil] = useState(regra.contaContabil || '');
