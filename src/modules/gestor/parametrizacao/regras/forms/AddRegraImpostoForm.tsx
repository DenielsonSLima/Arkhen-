...
interface AddRegraImpostoFormProps {
  cnaes: Cnae[];
...
export const AddRegraImpostoForm: React.FC<AddRegraImpostoFormProps> = ({ cnaes, onSave, onCancel, isSaving }) => {
  const [nome, setNome] = useState('');
  const [regime, setRegime] = useState<'Lucro Presumido' | 'Lucro Real'>('Lucro Presumido');
  const [cnaeCodigo, setCnaeCodigo] = useState(cnaes[0]?.codigo || '');
  const [cstPis, setCstPis] = useState('01');
  const [aliquotaPis, setAliquotaPis] = useState('0.65');
  const [cstCofins, setCstCofins] = useState('01');
  const [aliquotaCofins, setAliquotaCofins] = useState('3.00');
