...
export const SimulacoesCalculosPage: React.FC = () => {
  ...
  const handleGenerateShareLink = () => {
    ...
    const pwdParam = sharePasswordRequired && sharePassword ? `&pwd=${encodeURIComponent(sharePassword)}` : '';
    const expParam = `&exp=${shareExpires}`;
    const link = `https://compartilhar.auracontabil.com.br/simulacao/${abaAtiva}_${randomId}?utm_source=export${pwdParam}${expParam}`;
  ...
  const handleDownloadPdf = () => {
    ...
    setTimeout(() => {
      ...
      const docName = `Simulacao_${abaAtiva}_Aura_Contabilidade.pdf`;
      const blob = new Blob([`--- SIMULACAO AURA CONTABILIDADE ---\nRelatório: ${title}\nData: ${new Date().toLocaleString('pt-BR')}\nEmpresa: ${empresa?.razaoSocial || 'Aura Contabilidade'}\nCNPJ: ${empresa?.cnpj || '12.345.678/0001-90'}\n\nDocumento gerado com sucesso.`], { type: 'application/pdf' });
  ...
  const handleDiscardPdf = () => {
    if (confirm('Tem certeza que deseja descartar este relatório de simulação?')) {
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <table style={{ width: '100%', fontSize: '0.75rem', borderCollapse: 'collapse', textAlign: 'left' }}>
              ...
              <tbody>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><td style={{ padding: '8px', fontWeight: 500, color: '#334155' }}>Salário Base</td><td style={{ padding: '8px', textAlign: 'right', color: '#334155' }}>{formatCurrency(resultadoFolha.salarioBruto)}</td><td style={{ padding: '8px', textAlign: 'right', color: '#94a3b8' }}>-</td></tr>
      ...
      case 'rescisao':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <table style={{ width: '100%', fontSize: '0.75rem', borderCollapse: 'collapse', textAlign: 'left' }}>
              <tbody>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ paddingTop: '6px', paddingBottom: '6px', fontWeight: 600, color: '#64748b' }}>Motivo Rescisão</th><td style={{ paddingTop: '6px', paddingBottom: '6px', textAlign: 'right', fontWeight: 500, color: '#0f172a' }}>{rescisaoParams.tipo.replaceAll('_', ' ').toUpperCase()}</td></tr>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ paddingTop: '6px', paddingBottom: '6px', fontWeight: 600, color: '#64748b' }}>Aviso Prévio</th><td style={{ paddingTop: '6px', paddingBottom: '6px', textAlign: 'right', fontWeight: 500, color: '#0f172a' }}>{rescisaoParams.avisoPrevioModo.toUpperCase()}</td></tr>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ paddingTop: '6px', paddingBottom: '6px', fontWeight: 600, color: '#64748b' }}>Data de Admissão</th><td style={{ paddingTop: '6px', paddingBottom: '6px', textAlign: 'right', fontWeight: 500, color: '#0f172a' }}>{rescisaoParams.dataAdmissao}</td></tr>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ paddingTop: '6px', paddingBottom: '6px', fontWeight: 600, color: '#64748b' }}>Data de Desligamento</th><td style={{ paddingTop: '6px', paddingBottom: '6px', textAlign: 'right', fontWeight: 500, color: '#0f172a' }}>{rescisaoParams.dataDemissao}</td></tr>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ paddingTop: '6px', paddingBottom: '6px', fontWeight: 600, color: '#64748b' }}>Salário Base</th><td style={{ paddingTop: '6px', paddingBottom: '6px', textAlign: 'right', fontWeight: 700, color: '#0f172a' }}>{formatCurrency(parseCurrencyInputValue(rescisaoParams.salario))}</td></tr>
              </tbody>
            </table>

            <h4 style={{ fontSize: '0.88rem', fontWeight: 700, color: '#1e293b', borderBottom: '1px solid #cbd5e1', paddingBottom: '6px', margin: '12px 0 0 0' }}>VERBAS RESCISÓRIAS CALCULADAS</h4>
            <table style={{ width: '100%', fontSize: '0.75rem', borderCollapse: 'collapse', textAlign: 'left' }}>
              <tbody>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ padding: '8px 0', fontWeight: 600, color: '#64748b' }}>Saldo de Salário</th><td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 500, color: '#0f172a' }}>{formatCurrency(resultadoRescisao.saldoSalario)}</td></tr>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ padding: '8px 0', fontWeight: 600, color: '#64748b' }}>13º Salário Proporcional</th><td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 500, color: '#0f172a' }}>{formatCurrency(resultadoRescisao.decimoTerceiroProporcional)}</td></tr>
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ padding: '8px 0', fontWeight: 600, color: '#64748b' }}>Férias Proporcionais + 1/3</th><td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 500, color: '#0f172a' }}>{formatCurrency(resultadoRescisao.feriasProporcionais + resultadoRescisao.adicionalFerias)}</td></tr>
                {resultadoRescisao.avisoPrevio > 0 && (
                  <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ padding: '8px 0', fontWeight: 600, color: '#64748b' }}>Aviso Prévio Indenizado</th><td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 500, color: '#0f172a' }}>{formatCurrency(resultadoRescisao.avisoPrevio)}</td></tr>
                )}
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ padding: '8px 0', fontWeight: 600, color: '#64748b' }}>Multa Rescisória FGTS (40%)</th><td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 600, color: '#c59235' }}>{formatCurrency(resultadoRescisao.multaFGTS)}</td></tr>
              </tbody>
            </table>

            <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0', marginTop: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.75rem' }}><span style={{ fontWeight: 700, color: '#334155' }}>Total Líquido Rescisório:</span><span style={{ fontWeight: 800, color: '#c59235', fontSize: '1.1rem' }}>{formatCurrency(resultadoRescisao.totalLiquido)}</span></div>
            </div>
          </div>
        );
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <table style={{ width: '100%', fontSize: '0.75rem', borderCollapse: 'collapse', textAlign: 'left' }}>
              <tbody>
                ...
                <tr style={{ borderBottom: '1px solid #f1f5f9' }}><th style={{ padding: '8px 0', fontWeight: 600, color: '#64748b' }}>Escritório Emissor</th><td style={{ padding: '8px 0', textAlign: 'right', fontWeight: 500, color: '#0f172a' }}>{empresa?.nomeFantasia || 'Aura Contabilidade'}</td></tr>
              </tbody>
            </table>
          </div>
        );
        ...
        {isPdfModalOpen && (
          <div className="simulation-export-backdrop">
            ...
            >
              ...
              <div
                id="simulation-pdf-document"
                className="simulation-export-preview"
                style={{
                  flex: 1,
              ...
              >
                ...
                <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: '20px', height: '100%' }}>
                    ...
                    <div style={{ flexShrink: 0 }}>
                      {empresa?.logoUrl ? (
                        <img 
                          src={empresa.logoUrl} 
                          alt="Logo da Empresa" 
                          style={{ maxHeight: '56px', maxWidth: '160px', objectFit: 'contain', borderRadius: '4px' }} 
                        />
                      ) : (
                        <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#c59235', display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <Landmark size={22} />
                          <span>AURA CONTÁBIL</span>
                        </div>
                      )}
                    </div>

                    {/* Dados da empresa do lado direito */}
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2px', minWidth: 0 }}>
                      <h1 style={{ fontSize: '1.05rem', fontWeight: 800, color: '#0f172a', margin: 0, letterSpacing: '-0.02em', textTransform: 'uppercase' }}>
                        {empresa?.razaoSocial || 'Aura Contabilidade e Assessoria Ltda'}
                      </h1>
                      
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 12px', fontSize: '0.65rem', color: '#475569', fontWeight: 500 }}>
                        <span><strong>CNPJ:</strong> {formatCnpj(empresa?.cnpj || '12.345.678/0001-90')}</span>
                        <span><strong>Telefone:</strong> {empresa?.telefone || '(11) 4567-8901'}</span>
                        <span><strong>E-mail:</strong> {empresa?.email || 'contato@auracontabil.com.br'}</span>
                      </div>

                      <div style={{ fontSize: '0.62rem', color: '#64748b', fontWeight: 400, marginTop: '2px' }}>
                  ...
                  <div style={{
                    ...
                    <span>Gerado eletronicamente por Aura ERP em {new Date().toLocaleDateString('pt-BR')}</span>
              ...
              <div className="simulation-export-actions" style={{
                ...
                <div>
                  ...
                  <p style={{ fontSize: '0.78rem', color: '#94a3b8', lineHeight: '1.4', marginBottom: '24px' }}>
                    Escolha uma das ações abaixo para gerenciar o relatório gerado. Você pode baixar em PDF, excluir permanentemente do histórico temporário, ou criar um link seguro de compartilhamento para o cliente.
                  </p>
                      ...
                      {generatedLink && (
                        <div style={{
                          ...
                          <p style={{ fontSize: '0.6rem', color: '#94a3b8', margin: 0, display: 'flex', alignItems: 'center', gap: '4px' }}>
                            <Clock size={10} />
                            {shareExpires === 'never' ? 'Acesso permanente.' : `Expira em ${shareExpires}.`}
                            {sharePasswordRequired && ' Protegido.'}
                          </p>
                        </div>
                      )}
                    </div>
                ...
                <div style={{
                  ...
                  <span>Desenvolvido por Aura ERP</span>
                </div>
              </div>
