...
import { CONFORMIDADE_TEMPLATES, type ConformidadeTemplate } from './conformidadeTemplates';
...
const isMissingConformidadeTableError = (error: unknown): boolean => {
  ...
  return err.code === '42P01'
    || err.code === 'PGRST205'
    || message.includes('relation "public.conformidade_obrigacoes" does not exist')
    || message.includes("Could not find the table 'public.conformidade_obrigacoes'");
};
...
const buildSeedObrigacoes = async () => {
  ...
  for (const company of companies) {
    for (const template of CONFORMIDADE_TEMPLATES) {
