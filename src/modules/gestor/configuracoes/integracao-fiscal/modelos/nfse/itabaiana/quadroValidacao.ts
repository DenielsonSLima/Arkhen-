...
function fundoCinza(pdf: jsPDF, x: number, y: number) {
  const width = 47, height = 60;
  ...
  for (let line = 0; line < 140; line += 1) pdf.line(x + 4, y + 2 + line * 0.4, x + 6.3, y + 2 + line * 0.4);
  ...
  pdf.text('MUNICÍPIO DE ITABAIANA', x + 9.1, y + 57.8, { angle: 90 });
...
export function drawValidationPanel(pdf: jsPDF, nfse: NfseFiscalData, options: NfseModeloOptions, top: number) {
  ...
  pdf.setFillColor('#ffffff').rect(x + 11, y + 54.5, 34.5, 4, 'F');
  pdf.setFont('helvetica', 'normal').setFontSize(4.8).setTextColor('#111111');
  pdf.text(pdf.splitTextToSize(qr.caption, 33), center, y + 56, { align: 'center', lineHeightFactor: 1.1 });
}
