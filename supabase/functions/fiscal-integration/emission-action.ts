import { assertCertificateMatchesCnpj, parseFiscalPkcs12 } from "../_shared/webiss/certificate.ts";
import { consultWebIssNfse, emitWebIssNfse, prepareSignedWebIssRps } from "../_shared/webiss/emission.ts";
...
const dependencies = { consult: consultWebIssNfse, emit: emitWebIssNfse, sign: prepareSignedWebIssRps, parseCertificate: parseFiscalPkcs12 };
...
export async function handleEmissionAction(
  ...
  if (prepared.jaEmitida === true) {
    return { ok: true, success: true, nfseId: text(prepared.nfseId), jaEmitida: true, ambiente };
  }
  if (!/^[0-9a-f-]{36}$/i.test(text(prepared.tentativaId)) || typeof prepared.reconciliarPrimeiro !== "boolean") {
    throw new Error("Contrato de emissao segura ausente. Aplique a migration WebISS antes de transmitir notas.");
  }
  let sent = false;
  ...
  try {
    ...
    return {
      ok: true, success: true, nfseId, protocolo: result.protocolo, ambiente,
      reconciliada: mustConsult,
      message: ambiente === "homologacao" ? "NFS-e de homologacao registrada; sem efeito fiscal de producao." : "NFS-e confirmada no WebISS.",
    };
  } catch (error) {
    const message = error instanceof Error ? error.message : "Falha na operacao WebISS.";
    // Read-only consultation must never release another request's active lease.
    if (!consultOnly && prepared.tentativaId) {
      const status = mustConsult ? "incerta" : !sent ? "falha_pre_envio"
        : error instanceof WebIssError && error.definitiveRejection ? "rejeitada" : "incerta";
      const { error: finishError } = await admin.rpc("finalizar_tentativa_nfse_webiss", {
        p_user_id: userId, p_cobranca_id: chargeId, p_tentativa_id: prepared.tentativaId,
        p_status: status, p_mensagem: message,
      });
      if (finishError) throw new Error(`${message} Falha ao registrar o estado local; consulte o RPS antes de tentar novamente.`);
    }
    if (mustConsult) throw new Error(`${message} Nenhuma nova emissao foi enviada. Resolva a situacao do RPS no WebISS antes de reenviar.`);
    throw error;
  }
}
