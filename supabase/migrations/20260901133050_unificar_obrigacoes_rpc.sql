-- Contrato atômico de leitura e gravação da obrigação canônica.
...
BEGIN
  ...
  IF jsonb_typeof(p_payload) IS DISTINCT FROM 'object'
     ...
     OR COALESCE(p_payload ->> 'periodicidade', '') NOT IN (
       'mensal', 'quinzenal', 'trimestral', 'semestral'
     )
     OR COALESCE(p_payload ->> 'origemPadrao', '') NOT IN (
  ...
  WHERE valor IN ('PF', 'MEI', 'Simples Nacional', 'Lucro Presumido', 'Lucro Real', 'Isenta', 'Isento');
  ...
  IF (p_payload ? 'diaVencimento' AND app_private.jsonb_inteiro_entre(
        p_payload -> 'diaVencimento', 1, 31
      ) IS NULL)
     OR (p_payload ? 'diaPrimeiraQuinzena' AND app_private.jsonb_inteiro_entre(
       p_payload -> 'diaPrimeiraQuinzena', 1, 31
     ) IS NULL)
     OR (p_payload ? 'diaSegundaQuinzena' AND app_private.jsonb_inteiro_entre(
       p_payload -> 'diaSegundaQuinzena', 1, 31
     ) IS NULL) THEN
    RAISE EXCEPTION 'O dia de vencimento deve ficar entre 1 e 31.' USING ERRCODE = '22023';
  END IF;
  v_dia := COALESCE(app_private.jsonb_inteiro_entre(
    p_payload -> 'diaVencimento', 1, 31
  ), 20);
  ...
  IF v_tem_vencimento
     AND p_payload ->> 'periodicidade' = 'quinzenal'
     AND v_dia_primeira >= v_dia_segunda THEN
    RAISE EXCEPTION 'O vencimento da 1ª quinzena deve anteceder o da 2ª.'
      USING ERRCODE = '22023';
  END IF;

  v_periodicidade := p_payload ->> 'periodicidade';
  v_origem := p_payload ->> 'origemPadrao';

  -- Serializa catálogo e configuração de parceiros por tenant antes de qualquer
  -- row lock, evitando que uma rotina seja criada com metadados ultrapassados.
  ...
  IF NULLIF(p_payload ->> 'id', '') IS NOT NULL THEN
    BEGIN
      ...
      v_atualizado_em := NULLIF(p_payload ->> 'atualizadoEm', '')::timestamptz;
    ...
    IF NULLIF(p_payload ->> 'atualizadoEm', '') IS NULL
       OR v_atualizado_em IS DISTINCT FROM (p_payload ->> 'atualizadoEm')::timestamptz THEN
  ...
  INSERT INTO public.atividades_modelos AS modelo (
    id, empresa_id, codigo, nome, descricao, categoria, tipos, etapas, sistema, ativo, ordem
  ) VALUES (
    COALESCE(v_modelo_id, gen_random_uuid()), v_empresa_id, v_codigo, v_nome, v_descricao,
  ...
  INSERT INTO public.parametrizacao_protocolos_tipos AS tipo (
    id, empresa_id, codigo, nome, categoria, orgao, dia_limite, descricao, regimes,
    ...
    v_id, v_empresa_id, v_codigo, v_nome, v_categoria, v_orgao, v_dia, v_descricao,
    v_regimes, v_periodicidade, v_origem, false, v_ativo, v_etapas, v_tem_vencimento,
    v_referencia_anterior, v_dia_primeira, v_dia_segunda, 100, v_modelo_id
  )
  ...
  -- Bloqueia cliente e configuração na mesma ordem usada pela gravação CAS.
  -- Depois atualiza somente a projeção do card alterado, sem recalcular todas as
  -- agendas do cliente dentro desta RPC global.
  FOR v_cliente IN
    SELECT cliente.id
    ...
    WHERE cliente.empresa_id = v_empresa_id
      AND EXISTS (
        SELECT 1 FROM jsonb_array_elements(COALESCE(cfg.configs, '[]'::jsonb)) item(valor)
  ...
  SELECT jsonb_build_object(
    'id', tipo.id::text, 'codigo', tipo.codigo, 'nome', tipo.nome,
    'categoria', tipo.categoria, 'orgao', COALESCE(tipo.orgao, ''),
    'descricao', COALESCE(tipo.descricao, ''), 'regimes', to_jsonb(tipo.regimes),
    'periodicidade', tipo.periodicidade_padrao,
    'origemPadrao', CASE tipo.origem_padrao WHEN 'cliente' THEN 'Cliente envia'
      WHEN 'escritorio' THEN 'Escritório envia' WHEN 'ambos' THEN 'Ambos'
      ELSE tipo.origem_padrao END,
    'temVencimento', tipo.tem_vencimento, 'diaVencimento', tipo.dia_limite,
    'referenciaMesAnterior', tipo.referencia_mes_anterior,
    'diaPrimeiraQuinzena', tipo.dia_vencimento_primeira_quinzena,
    'diaSegundaQuinzena', tipo.dia_vencimento_segunda_quinzena,
    'etapas', tipo.etapas, 'ativo', tipo.ativo, 'ordem', tipo.ordem,
    'atualizadoEm', tipo.atualizado_em::text
  ) INTO v_resultado
  FROM public.parametrizacao_protocolos_tipos tipo
  WHERE tipo.id = v_id AND tipo.empresa_id = v_empresa_id;
...
-- A partir daqui a RPC canônica já existe; as telas antigas não podem manter
-- uma segunda fonte de escrita durante um deploy parcialmente concluído.
