...
begin
  ...
  if v_escopo_cliente then
    select coalesce(jsonb_agg(
      jsonb_build_object(
        'id',c.id,'nome',c.nome,'razaoSocial',c.razao_social,
        'cnpj',c.cnpj,'logo',c.logo,'status',c.status
      ) order by c.nome
    ),'[]'::jsonb)
    into v_clientes
  ...
  return jsonb_build_object(
    ...
    'clientes',v_clientes,
    'identidadeNome',case when v_escopo_cliente
      then coalesce(v_cliente_principal->>'nome',v_empresa_nome)
      else v_empresa_nome end,
    'identidadeLogo',case when v_escopo_cliente
      then v_cliente_principal->>'logo' else null end
  );
end;
$$;
