-- Persiste todas as configuracoes visuais por orientacao. A tela ja usa esses
-- campos; esta migration elimina a divergencia entre preview, RPC e PDF.
...
SET
  posicao_paisagem = CASE
    WHEN posicao_paisagem IN ('topo-esquerda', 'topo-direita', 'centro', 'rodape-direita')
      THEN posicao_paisagem
    WHEN posicao IN ('topo-esquerda', 'topo-direita', 'centro', 'rodape-direita')
      THEN posicao
    ELSE 'centro'
  END,
  posicao_retrato = CASE
    WHEN posicao_retrato IN ('topo-esquerda', 'topo-direita', 'centro', 'rodape-direita')
      THEN posicao_retrato
    WHEN posicao IN ('topo-esquerda', 'topo-direita', 'centro', 'rodape-direita')
      THEN posicao
    ELSE 'centro'
  END,
  opacidade_paisagem = LEAST(100, GREATEST(0, COALESCE(opacidade_paisagem, opacidade, 15))),
  opacidade_retrato = LEAST(100, GREATEST(0, COALESCE(opacidade_retrato, opacidade, 15))),
  tamanho_paisagem = LEAST(100, GREATEST(10, COALESCE(tamanho_paisagem, tamanho, 35))),
  tamanho_retrato = LEAST(100, GREATEST(10, COALESCE(tamanho_retrato, tamanho, 35)));
...
BEGIN
  ...
  INSERT INTO public.configuracoes_marca_dagua (
    ...
    posicao, opacidade, tamanho,
    posicao_paisagem, posicao_retrato,
    opacidade_paisagem, opacidade_retrato,
    tamanho_paisagem, tamanho_retrato
  ) VALUES (
    v_empresa_id,
