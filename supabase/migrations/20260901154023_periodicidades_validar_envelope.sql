CREATE OR REPLACE FUNCTION app_private.validar_envelope_configs_protocolos_cliente(
  p_empresa_id uuid,
  p_regime text,
  p_configs jsonb
)
...
AS $$
BEGIN
  IF jsonb_typeof(p_configs) IS DISTINCT FROM 'array'
  ...
  IF EXISTS (
    SELECT 1 FROM jsonb_array_elements(p_configs) item(valor)
    WHERE jsonb_typeof(item.valor -> 'entregaId') IS DISTINCT FROM 'string'
      ...
      OR NOT EXISTS (
        ...
        WHERE tipo.empresa_id = p_empresa_id
          ...
          AND (item.valor ->> 'ativo' = 'false'
            OR (tipo.ativo = true AND p_regime = ANY(tipo.regimes)))
      )
  ) OR EXISTS (
    SELECT 1 FROM jsonb_array_elements(p_configs) item(valor)
...
REVOKE ALL ON FUNCTION app_private.validar_envelope_configs_protocolos_cliente(
  uuid, text, jsonb
